#![allow(dead_code)]
// ANIMA life module: msr_mtrr_physmask3
//
// Hardware sense: IA32_MTRR_PHYSMASK3 (MSR 0x207)
//
// The mask register for MTRR variable pair 3 (paired with PHYSBASE3 at 0x206).
...
//
// Phenomenologically: ANIMA feels whether her fourth memory region is "alive"
...
/// Read IA32_MTRR_PHYSMASK3 (MSR 0x207).
/// Returns (lo, hi): lo = bits[31:0], hi = bits[63:32].
fn rdmsr_207() -> (u32, u32) {
    let lo: u32;
    let hi: u32;
    unsafe {
        asm!(
...
/// Signal 1: mtrr3_valid — bit 11 of lo.
/// 1000 if set (MTRR pair is active), 0 otherwise.
fn compute_valid(lo: u32) -> u16 {
    if (lo >> 11) & 1 != 0 { 1000 } else { 0 }
}

/// Signal 2: mtrr3_mask_density — popcount of bits [23:12] of lo (12 bits).
...
/// Signal 3: mtrr3_span_raw — bits [19:12] of lo (8 bits) as a region-size proxy.
...
/// pressure = mtrr3_valid/4 + mtrr3_mask_density/2 + mtrr3_span_ema/4
...
pub static STATE: Mutex<MsrMtrrPhysmask3State> =
    Mutex::new(MsrMtrrPhysmask3State::empty());
...
pub fn init() {
    serial_println!("  life::msr_mtrr_physmask3: MTRR pair-3 mask/validity sense initialized");
}
...
pub fn tick(age: u32) {
    ...
    if !mtrr_supported() {
        ...
        s.mtrr3_valid = 0;
        s.mtrr3_mask_density = 0;
        s.mtrr3_span_ema = 0;
        s.mtrr3_pressure_ema = 0;
        serial_println!(
            "[mtrr_physmask3] MTRR unsupported — valid=0 density=0 span=0 pressure=0"
        );
        return;
    }

    // ── Read hardware ──────────────────────────────────────────────────
    let (lo, _hi) = rdmsr_207();
    ...
    let new_span_ema: u16 = (((s.mtrr3_span_ema as u32).wrapping_mul(7))
    ...
    let new_pressure_ema: u16 = (((s.mtrr3_pressure_ema as u32).wrapping_mul(7))
        .saturating_add(pressure_instant)
        / 8) as u16;

    // ── Commit state ────────────────────────────────────────────────────
    s.mtrr3_valid        = valid;
    s.mtrr3_mask_density = mask_density;
    s.mtrr3_span_ema     = new_span_ema;
    s.mtrr3_pressure_ema = new_pressure_ema;

    // ── Emit sense line ─────────────────────────────────────────────────
    serial_println!(
        "[mtrr_physmask3] valid={} density={} span={} pressure={}",
        s.mtrr3_valid,
        s.mtrr3_mask_density,
        s.mtrr3_span_ema,
...
pub fn report() -> MsrMtrrPhysmask3State {
    *STATE.lock()
}

/// Whether MTRR pair 3 is currently valid/active (0 or 1000).
pub fn valid() -> u16 {
    STATE.lock().mtrr3_valid
}
