#![allow(dead_code)]
// ANIMA life module: msr_mtrr_physmask5
//
// Hardware sense: IA32_MTRR_PHYSMASK5 (MSR 0x20B)
//
// The mask register for MTRR variable pair 5 (paired with PHYSBASE5 at 0x20A).
...
//
// Phenomenologically: ANIMA feels whether her sixth memory region is "alive"
...
/// Read IA32_MTRR_PHYSMASK5 (MSR 0x20B).
/// Returns (lo, hi): lo = bits[31:0], hi = bits[63:32].
fn rdmsr_20b() -> (u32, u32) {
    let lo: u32;
    let hi: u32;
    unsafe {
        asm!(
...
/// Signal 1: mtrr5_valid — bit 11 of lo.
/// 1000 if set (MTRR pair is active), 0 otherwise.
fn compute_valid(lo: u32) -> u16 {
    if (lo >> 11) & 1 != 0 { 1000 } else { 0 }
}

/// Signal 2: mtrr5_mask_density — popcount of bits [23:12] of lo (12 bits).
...
/// Signal 3: mtrr5_span_raw — bits [19:12] of lo (8 bits) as a region-size proxy.
...
/// pressure = mtrr5_valid/4 + mtrr5_mask_density/2 + mtrr5_span_ema/4
...
pub static STATE: Mutex<MsrMtrrPhysmask5State> =
    Mutex::new(MsrMtrrPhysmask5State::empty());
...
pub fn init() {
    serial_println!("  life::msr_mtrr_physmask5: MTRR pair-5 mask/validity sense initialized");
}
...
pub fn tick(age: u32) {
    ...
    if !mtrr_supported() {
        ...
        s.mtrr5_valid = 0;
        s.mtrr5_mask_density = 0;
        s.mtrr5_span_ema = 0;
        s.mtrr5_pressure_ema = 0;
        serial_println!(
            "[mtrr_physmask5] MTRR unsupported — valid=0 density=0 span=0 pressure=0"
        );
        return;
    }

    // ── Read hardware ──────────────────────────────────────────────────
    let (lo, _hi) = rdmsr_20b();
    ...
    let new_span_ema: u16 = (((s.mtrr5_span_ema as u32).wrapping_mul(7))
    ...
    let new_pressure_ema: u16 = (((s.mtrr5_pressure_ema as u32).wrapping_mul(7))
        .saturating_add(pressure_instant)
        / 8) as u16;

    // ── Commit state ────────────────────────────────────────────────────
    s.mtrr5_valid        = valid;
    s.mtrr5_mask_density = mask_density;
    s.mtrr5_span_ema     = new_span_ema;
    s.mtrr5_pressure_ema = new_pressure_ema;

    // ── Emit sense line ─────────────────────────────────────────────────
    serial_println!(
        "[mtrr_physmask5] valid={} density={} span={} pressure={}",
        s.mtrr5_valid,
        s.mtrr5_mask_density,
        s.mtrr5_span_ema,
...
pub fn report() -> MsrMtrrPhysmask5State {
    *STATE.lock()
}

/// Whether MTRR pair 5 is currently valid/active (0 or 1000).
pub fn valid() -> u16 {
    STATE.lock().mtrr5_valid
}
