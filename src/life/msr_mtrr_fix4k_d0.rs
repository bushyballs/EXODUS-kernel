// msr_mtrr_fix4k_d0.rs — IA32_MTRR_FIX4K_D0000 MSR (0x26B)
// Fixed-range MTRR for the 32KB region 0xD0000–0xD7FFF (Upper Memory Area).
// The MSR encodes eight 4KB sub-ranges as one byte each:
//   lo = [D0 D1 D2 D3]  (0xD0000–0xD3FFF, 4 sub-ranges)
//   hi = [D4 D5 D6 D7]  (0xD4000–0xD7FFF, 4 sub-ranges)
// Each byte is a memory type code:
//   0 = UC  (Uncacheable)
//   1 = WC  (Write-Combining)
//   4 = WT  (Write-Through)
//   5 = WP  (Write-Protect)
//   6 = WB  (Write-Back)      — common in upper memory when shadowed
//
// Signals (all u16, 0–1000):
//   fix4k_d0_wb  — count of WB (6) bytes across all 8 sub-ranges, scaled *125
//   fix4k_d0_uc  — count of UC (0) bytes across all 8 sub-ranges, scaled *125
//   fix4k_d0_mix — popcount(lo ^ hi) * 31, capped at 1000
//                  measures diversity between the low and high 4KB range halves
//   fix4k_d0_ema — EMA of ((fix4k_d0_wb / 2 + fix4k_d0_mix / 2).min(1000))
//
...
#[derive(Clone, Copy)]
pub struct MsrMtrrFix4kD0State {
    ...
    pub fix4k_d0_mix: u16,
    /// EMA of ((fix4k_d0_wb / 2 + fix4k_d0_mix / 2).min(1000)).
    pub fix4k_d0_ema: u16,
    /// Tick counter (drives sample gate).
    pub age: u32,
}

impl MsrMtrrFix4kD0State {
...
static STATE: Mutex<MsrMtrrFix4kD0State> = Mutex::new(MsrMtrrFix4kD0State::new());
...
// ---------------------------------------------------------------------------
// rdmsr helper — reads IA32_MTRR_FIX4K_D0000 (0x26B).
// Returns (lo, hi): lo = bits[31:0], hi = bits[63:32].
// ---------------------------------------------------------------------------

#[inline]
unsafe fn rdmsr_0x26b() -> (u32, u32) {
    let lo: u32;
    let hi: u32;
    core::arch::asm!(
        "rdmsr",
...
pub fn init() {
    crate::serial_println!(
        "  life::msr_mtrr_fix4k_d0: upper-memory fixed-range MTRR sense (0xD0000-0xD7FFF) initialized"
    );
}
...
pub fn tick(age: u32) {
    ...
    if !mtrr_supported() {
        state.fix4k_d0_wb  = 0;
        state.fix4k_d0_uc  = 0;
        state.fix4k_d0_mix = 0;
        state.fix4k_d0_ema = 0;
        crate::serial_println!(
            "[msr_mtrr_fix4k_d0] tick={} MTRR not supported — all signals zeroed",
            age
        );
        return;
    }
    ...
    // Read MSR 0x26B.
    let (lo, hi) = unsafe { rdmsr_0x26b() };

    // Extract the 8 memory-type bytes.
    let lanes = extract_lanes(lo, hi);

    // --- fix4k_d0_wb: count WB (6) bytes, scale *125 ---
    let fix4k_d0_wb = count_type(&lanes, 6);

    // --- fix4k_d0_uc: count UC (0) bytes, scale *125 ---
    let fix4k_d0_uc = count_type(&lanes, 0);
    ...
    let fix4k_d0_mix: u16 = (ones.wrapping_mul(31)).min(1000) as u16;

    // --- fix4k_d0_ema: EMA of ((fix4k_d0_wb / 2 + fix4k_d0_mix / 2).min(1000)) ---
    // Divide each component by 2 before adding to avoid overflow above 1000.
    let half_wb: u32 = (fix4k_d0_wb as u32) / 2;
    let half_mix: u32 = (fix4k_d0_mix as u32) / 2;
    let ema_input: u16 = half_wb.saturating_add(half_mix).min(1000) as u16;
    let fix4k_d0_ema = ema_u16(state.fix4k_d0_ema, ema_input);

    // Commit.
    state.fix4k_d0_wb  = fix4k_d0_wb;
    state.fix4k_d0_uc  = fix4k_d0_uc;
    state.fix4k_d0_mix = fix4k_d0_mix;
    state.fix4k_d0_ema = fix4k_d0_ema;

    crate::serial_println!(
        "[msr_mtrr_fix4k_d0] tick={} wb={} uc={} mix={} ema={} (lo={:#010x} hi={:#010x})",
        age,
        fix4k_d0_wb,
        fix4k_d0_uc,
        fix4k_d0_mix,
